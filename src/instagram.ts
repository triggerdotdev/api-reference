...
// Create a access token at developers.facebook.com
const token = {
  ...
  run: async(payload, io, ctx) => {
    ...
    await io.runTask(
      ...
      async () => {
        ...
        const createContainerUrl = `${endpointURLCreate}?image_url=${payload.imageUrl}&caption=${encodeURIComponent(payload.caption)}&access_token=${token.accessToken}`;
